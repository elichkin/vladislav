...
### Напишите функцию, которой на вход подаётся кортеж из чисел. Функция должна вернуть кортеж только с нечетными числами.
...
![Результат решения](./pic/Samost6_5.PNG)

## Выводы

В данном задании, сначала я привел кортеж к списку, итерируюсь по нему и проверяю значение на нечетность. Далее сформирую новый список, который после преобразую в кортеж.

## Общий вывод по самостоятельной работе №6

1. В Python кортежи, как и списки, предназначены для хранения последовательностей, состоящих из данных любого типа.
2. Однако, в отличие от списков, кортежи относятся к неизменяемым типам данных.
